...
run_tests_per_cluster <- function(df, nutrient) {
  ...
  if (n_distinct(df$sampling_type) < 2) return(NULL)  # Need both SP and C
  
  # Check normality per sampling_type
  normality <- df %>%
...
# Apply the renaming to lime_long (or whichever dataframe you use)
lime_long <- lime_long %>%
  mutate(cluster_name = cluster_names[as.character(cluster)]) %>%
  mutate(cluster_name = factor(cluster_name, levels = cluster_names))  # keep order

# Then use cluster_name instead of cluster in your ggplot:
