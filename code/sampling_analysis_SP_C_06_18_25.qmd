  ---
title: "sampling_analysis_SP_C_06_18_25"
format: html
---
###Load Libraries

```{r load libraries}
library(tidyverse)
library(ggplot2)
library(janitor)
library(stringr)
library(dplyr)
library(leaflet)
library(sf)
library(patchwork)
library(purrr)
library(lme4)
library(lmerTest)
library(dbscan)
library(broom)
library(scales)
library(car)

```

### Load Data:

```{r data pull, echo=FALSE, message=FALSE, warning=FALSE}
# Load the data
lime <- read_csv("../data_do_not_push/2021 Liming Study Soil Data Compiled (lbs. A).csv") %>%
# Clean the data
  clean_names() %>%
  #split "sample" column by type and site (0-4,is type, Site 1,2... is site)
  separate(sample, into = c("type", "block"), sep = ", ") %>%
  #make sure site rows are lowercase and _ instead of spaces:
  mutate(site = tolower(site),
         site = str_replace_all(site, " ", "_")) %>%
  #group by site and the SAME lat long:
  group_by(site, lat, long) %>%
  arrange(lat, long) %>%
  #filter out 4-8 sample types:
  filter(type != "4-8") %>%
  mutate(location = cur_group_id()) %>%   # unique id for lat lon
  ungroup() %>%    
  arrange(location) %>%
  #add new column and copy over all 0-4 values, but change A and B to SP:
  mutate(sampling_type = case_when(
    type == "0-4" ~ "C",
    type == "A" ~ "SP",
    type == "B" ~ "SP",
    TRUE ~ type
  ))



```



#### Map Data:
```{r}

lime_sf <- lime %>%
  st_as_sf(coords = c("long", "lat"), crs = 4326) # WGS84 (lon/lat)

#create leaflet plot of lime_sf with points colored by type and labelled by location:

leaflet(lime_sf) %>%
  addTiles() %>%
  addCircleMarkers(
    color = ~case_when(
      type == "0-4" ~ "red",
      type == "A" ~ "green",
      type == "B" ~ "pink"),
    label = ~paste("Location:", location, "<br>Type:", type),
    radius = 5,
    stroke = FALSE,
    fillOpacity = 0.8
  ) %>%
  addLegend("bottomright", 
            colors = c("red", "green", "pink"),
            labels = c("0-4","A","B"),
            title = "Sample Type") %>%
  #change basemap to satellite:
  addProviderTiles("Esri.WorldImagery")
```
### Per site analysis

```{r Cluster by site}

# Step 1: Convert lime to sf (CRS 4326 for original lat/lon)
lime_sf <- lime %>%
  st_as_sf(coords = c("long", "lat"), crs = 4326)

# Step 2: Transform to UTM (meters) for distance-based clustering
lime_utm <- st_transform(lime_sf, crs = 32617)  # UTM Zone 17N

# Step 3: Extract coordinates for DBSCAN
coords <- st_coordinates(lime_utm)

# Step 4: Run DBSCAN clustering — try smaller eps (in meters)
# eps = 10000 is 10km, adjust based on how spread out your points are
clustering <- dbscan(coords, eps = 10000, minPts = 2)

# Step 5: Attach clusters back to original (in degrees)
lime_sf$cluster <- as.factor(clustering$cluster)  # back in CRS 4326

# Step 6: Visualize in Leaflet
leaflet(lime_sf) %>%
  addProviderTiles(providers$Esri.NatGeoWorldMap) %>%
  addCircleMarkers(
    radius = 5,
    color = ~colorFactor(rainbow(length(unique(lime_sf$cluster))), lime_sf$cluster)(cluster),
    stroke = FALSE,
    fillOpacity = 0.8,
    label = ~paste("Cluster:", cluster)
  )


```
```{r log transformed (aside from pH) and analyzed based on clustered sites}
# Define which nutrients to log-transform
nutrients <- c("ca", "k", "mg", "mn", "p", "zn", "p_h_2", "lbc_1", "lb_ceq")

nutrient_names <- c(
  "ca"     = "Calcium (Ca)",
  "k"      = "Potassium (K)",
  "mg"     = "Magnesium (Mg)",
  "mn"     = "Manganese (Mn)",
  "p"      = "Phosphorus (P)",
  "zn"     = "Zinc (Zn)",
  "p_h_2"  = "pH",
  "lbc_1"  = "Lime Buffer Capacity (LBC)",
  "lb_ceq" = "Lime Buffer Capacity Equivalent (LBCeq)"
)

nutrients_to_log <- setdiff(nutrients, "p_h_2")

# Apply log1p transformation to selected columns
lime_sf <- lime_sf %>%
  mutate(across(all_of(nutrients_to_log), ~ log1p(.)))

# Run your statistical tests as before
run_tests_per_cluster <- function(df, nutrient) {
  df <- df %>% filter(!is.na(.data[[nutrient]]))
  if (n_distinct(df$sampling_type) < 2) return(NULL)

  normality <- df %>%
    group_by(sampling_type) %>%
    summarise(p = shapiro.test(.data[[nutrient]])$p.value, .groups = "drop")

  use_t <- all(normality$p > 0.05)

  test_res <- if (use_t) {
    t.test(df[[nutrient]] ~ df$sampling_type)
  } else {
    wilcox.test(df[[nutrient]] ~ df$sampling_type)
  }

  tibble(
    cluster = unique(df$cluster),
    nutrient = nutrient,
    test = ifelse(use_t, "t-test", "wilcox"),
    p_value = test_res$p.value,
    statistic = test_res$statistic,
    median_C = median(df[[nutrient]][df$sampling_type == "C"], na.rm = TRUE),
    median_SP = median(df[[nutrient]][df$sampling_type == "SP"], na.rm = TRUE)
  )
}

results_by_cluster <- lime_sf %>%
  group_by(cluster) %>%
  group_map(~ map_dfr(nutrients, run_tests_per_cluster, df = .x), .keep = TRUE) %>%
  bind_rows() %>%
  arrange(nutrient)

print(results_by_cluster)

# ----------------------------
# Prep long data for plotting
# ----------------------------

# Region labels in the order you want them to appear
cluster_levels <- c("Lower Coastal Plains", "Upper Coastal Plains", "Piedmont", "Valley and Ridge")

# Prepare long data for plotting
cluster_names <- c(
  "1" = "Lower Coastal Plains",
  "2" = "Upper Coastal Plains",
  "3" = "Piedmont",
  "4" = "Valley and Ridge"
)

lime_long <- lime_sf %>%
  st_drop_geometry() %>%
  pivot_longer(
    cols = all_of(nutrients),
    names_to = "nutrient",
    values_to = "value"
  ) %>%
  mutate(
    # Pretty nutrient names for facets
    nutrient = recode(nutrient, !!!nutrient_names),
    nutrient = factor(nutrient, levels = unname(nutrient_names)),

    # Pretty region names + enforce region order
    cluster_name = cluster_names[as.character(cluster)],
    cluster_name = factor(cluster_name, levels = unname(cluster_names)),

    # Enforce sampling order + pretty sampling labels
    sampling_type = factor(sampling_type, levels = c("C", "SP")),
    sampling_type_full = factor(
      recode(as.character(sampling_type), "C" = "Composite", "SP" = "Single Point"),
      levels = c("Composite", "Single Point")
    ),

    # THIS is the key: sampling varies fastest within each region, so pairs are adjacent
    x_label = interaction(sampling_type_full, cluster_name, sep = " - ", drop = TRUE)
  )

# Plot (same look, just fixed grouping + clean labels)
p <- ggplot(lime_long, aes(x = x_label, y = value)) +
  geom_boxplot(aes(color = sampling_type), alpha = 0.3, outlier.shape = NA) +
  geom_jitter(aes(color = cluster_name), width = 0.2, alpha = 0.7, size = 1.5) +
  facet_wrap(~ nutrient, scales = "free_y") +
  scale_color_discrete(labels = c("C" = "Composite", "SP" = "Single Point")) +
  labs(
    x = "Sampling Type and Region",
    y = "Log-Transformed Nutrient Value (except pH)",
    title = "Soil Properties in Bermuda Hay Grasslands by Region and Sampling Type",
    color = "Region"
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "bottom",
    plot.margin = margin(10, 10, 10, 60)
  )

ggsave("../output_do_not_push/nutrient_boxplots_by_region.png", p,
       width = 12, height = 8, dpi = 300, units = "in", limitsize = FALSE)


# Composite samples have tighter distibutions, as to be expected.
```

```{r}
# plot distributions of all nutrients for sp and c samples:

lime_long %>%
  ggplot(aes(x = value, fill = sampling_type)) +
  geom_density(alpha = 0.5) +
  facet_wrap(~ nutrient, scales = "free") +
  scale_fill_manual(values = c("C" = "#1f77b4", "SP" = "#ff7f0e"),
                    labels = c("C" = "Composite", "SP" = "Single Point")) +
  labs(x = "Log-Transformed Nutrient Value (except pH)",
       y = "Density",
       title = "Density Plots of Bermuda Hay Grassland Soil Nutrients by Sampling Type") +
  theme_minimal(base_size = 14) +
  theme(legend.position = "bottom")
#save output:
ggsave("../output_do_not_push/nutrient_density_plots_by_sampling_type.png",
       width = 12, height = 8, dpi = 300, units = "in", limitsize = FALSE)
```


```{r}
#test for normality for each nutrient in lime_sf
normality_results <- map_dfr(nutrients, function(nutrient) {
  dat <- lime_sf %>% filter(!is.na(.data[[nutrient]]))

  c_val  <- dat[[nutrient]][dat$sampling_type == "C"]
  sp_val <- dat[[nutrient]][dat$sampling_type == "SP"]

  tibble(
    nutrient = nutrient,
    group = c("C", "SP"),
    n = c(length(c_val), length(sp_val)),
    shapiro_p = c(
      if (length(c_val)  >= 3) shapiro.test(c_val)$p.value  else NA,
      if (length(sp_val) >= 3) shapiro.test(sp_val)$p.value else NA
    )
  )
})
assumptions <- normality_results %>%
  pivot_wider(names_from = group, values_from = shapiro_p) %>%
  left_join(variance_results, by = "nutrient") %>%
  mutate(
    test_recommendation = case_when(
      C > 0.05 & SP > 0.05 & p_value > 0.05 ~ "Student t-test",
      C > 0.05 & SP > 0.05 & p_value <= 0.05 ~ "Welch t-test",
      TRUE ~ "Wilcoxon"
    )
  )

assumptions


```


### ANOVA assumptions check:

```{r ANOVA assumptions check, warning=FALSE}
# Check ANOVA assumptions: normality and homogeneity of variance
#Ca
# For normality
plot(aov(ca ~ type, data = lime))  # Residuals vs Fitted + Q-Q plot
# For equal variances
car::leveneTest(ca ~ type, data = lime)
#Mg
plot(aov(mg ~ type, data = lime))  # Residuals vs Fitted + Q-Q plot
car::leveneTest(mg ~ type, data = lime)
#Zn
plot(aov(zn ~ type, data = lime))  # Residuals vs Fitted + Q-Q plot
car::leveneTest(zn ~ type, data = lime)
```


```{r sampling analysis, echo=FALSE, message=FALSE, warning=FALSE}
#sample analysis
#compare o-4, A, and B samples by location using a statistical test for each nutrient:

# phosphorus (P)
print("Phosphorus (P) Analysis")
anova_p <- aov(p ~ type, data = lime)
summary(anova_p)
# potassium (K)
print("Potassium (K) Analysis")
anova_k <- aov(k ~ type, data = lime)
summary(anova_k)
# calcium (Ca)
print("Calcium (Ca) Analysis")
anova_ca <- aov(ca ~ type, data = lime)
summary(anova_ca)
# magnesium (Mg)
print("Magnesium (Mg) Analysis")
anova_mg <- aov(mg ~ type, data = lime)
summary(anova_mg)
# zinc (Zn)
print("Zinc (Zn) Analysis")
anova_zn <- aov(zn ~ type, data = lime)
summary(anova_zn)
# pH
print("pH Analysis")
anova_ph <- aov(p_h_2 ~ type, data = lime)
summary(anova_ph)
# lbc
print("Lime Buffer Capacity (LBC) Analysis")
anova_lbc <- aov(lbc_1 ~ type, data = lime)
summary(anova_lbc)
# lbc_eq
print("Lime Buffer Capacity Equivalent (LBC Eq) Analysis")
anova_lbc_eq <- aov(lb_ceq ~ type, data = lime)
summary(anova_lbc_eq)
```
```{r}
# Post-hoc tests for significant ANOVA results
print("Ca Post-hoc Test")
TukeyHSD(aov(ca ~ type, data = lime))
print("Mg Post-hoc Test")
TukeyHSD(aov(mg ~ type, data = lime))
print("P Post-hoc Test")
TukeyHSD(aov(zn ~ type, data = lime))

```
## Assumptions check:

```{r assumptions check, echo=FALSE, message=FALSE, warning=FALSE}
#check assummptions for t-test:
# Normality check
print("Normality Check for Nutrients by Sampling Type")

shapiro.test(lime$ca[lime$sampling_type == "C"])

shapiro.test(lime$ca[lime$sampling_type == "SP"])

shapiro.test(lime$mg[lime$sampling_type == "C"])

shapiro.test(lime$mg[lime$sampling_type == "SP"])

shapiro.test(lime$p[lime$sampling_type == "C"])
shapiro.test(lime$p[lime$sampling_type == "SP"])
shapiro.test(lime$zn[lime$sampling_type == "C"])
shapiro.test(lime$zn[lime$sampling_type == "SP"])
shapiro.test(lime$p_h_2[lime$sampling_type == "C"])
shapiro.test(lime$p_h_2[lime$sampling_type == "SP"])
shapiro.test(lime$lbc_1[lime$sampling_type == "C"])
shapiro.test(lime$lbc_1[lime$sampling_type == "SP"])
shapiro.test(lime$lb_ceq[lime$sampling_type == "C"])
shapiro.test(lime$lb_ceq[lime$sampling_type == "SP"])
shapiro.test(lime$k[lime$sampling_type == "C"])
shapiro.test(lime$k[lime$sampling_type == "SP"])



# Variance check
car::leveneTest(ca ~ sampling_type, data = lime)
car::leveneTest(mg ~ sampling_type, data = lime)
car::leveneTest(p ~ sampling_type, data = lime)
car::leveneTest(zn ~ sampling_type, data = lime)
car::leveneTest(p_h_2 ~ sampling_type, data = lime)
car::leveneTest(lbc_1 ~ sampling_type, data = lime)
car::leveneTest(lb_ceq ~ sampling_type, data = lime)
car::leveneTest(k ~ sampling_type, data = lime)



```



All nutrients fail normality in both C and SP groups (all p-values < 0.05).

Therefore, I should not use t-tests.

Most nutrients pass homogeneity of variance, except for one (possibly p_h_2 or k), which had p = 0.02083.

A test that takes into account for no normality:

## Wilcoxon Rank Sum Tests (Mann–Whitney U):

```{r Wilcoxon analysis}
# nutrients <- c("ca", "mg", "zn", "p", "k", "p_h_2", "lbc_1", "lb_ceq")
# 
# wilcox_results <- lapply(nutrients, function(nutrient) {
#   formula <- as.formula(paste(nutrient, "~ sampling_type"))
#   test <- wilcox.test(formula, data = lime)
#   
#   data.frame(
#     nutrient = nutrient,
#     W = test$statistic,
#     p_value = test$p.value,
#     median_C = median(lime[[nutrient]][lime$sampling_type == "C"], na.rm = TRUE),
#     median_SP = median(lime[[nutrient]][lime$sampling_type == "SP"], na.rm = TRUE)
#   )
# })
# 
# wilcox_df <- do.call(rbind, wilcox_results)
# wilcox_df

```


```{r Wilcoxon test with nested locations}

# Step 1: Average A and B to create one SP value per location
lime_avg <- lime %>%
  mutate(sampling_type = ifelse(type %in% c("A", "B"), "SP", sampling_type)) %>%
  group_by(location, sampling_type) %>%
  summarise(across(c(ca, k, mg, mn, p, zn, p_h_2, lbc_1, lb_ceq), mean, na.rm = TRUE), .groups = "drop")

# Step 2: Reshape so we can do paired comparisons
lime_wide <- lime_avg %>%
  pivot_wider(names_from = sampling_type, values_from = c(ca, k, mg, mn, p, zn, p_h_2, lbc_1, lb_ceq))

# Step 3: Paired Wilcoxon test for each nutrient
nutrients <- c("ca", "k", "mg", "mn", "p", "zn", "p_h_2", "lbc_1", "lb_ceq")

results <- map_dfr(nutrients, function(nutrient) {
  C_col <- lime_wide[[paste0(nutrient, "_C")]]
  SP_col <- lime_wide[[paste0(nutrient, "_SP")]]

  # Paired test
  test <- wilcox.test(C_col, SP_col, paired = TRUE, exact = FALSE)

  tibble(
    nutrient = nutrient,
    p_value = test$p.value,
    median_C = median(C_col, na.rm = TRUE),
    median_SP = median(SP_col, na.rm = TRUE),
    W = test$statistic
  )
})
results

```


Most nutrients (except K and pH) show statistically significant differences between C and SP, with higher medians in C samples.

This supports the idea that single points (SP) tend to underrepresent nutrient values compared to composite (C), which might influence lime or fertilizer recommendations.

pH is not significantly different between the types... interesting and potentially important for lime recommendations since pH is often the direct driver.

### Linear Mixed Effects Model:

```{r}



# Linear mixed-effects model to account for nested structure (calcium)

#sum to zero contrasts
#log:
lime_log<- lime %>%
  mutate(across(c(ca, k, mg, mn, p, zn, p_h_2, lbc_1, lb_ceq), log1p))

options(contrasts = c("contr.sum", "contr.poly"))

lmer_results <- map_dfr(nutrients, function(nutrient) {
  formula <- as.formula(paste(nutrient, "~ sampling_type + (1 | location)"))
  model <- lmer(formula, data = lime_log)
  
  anova_result <- anova(model)
  
  tibble(
    nutrient = nutrient,
    p_value = anova_result$`Pr(>F)`[1],
    F_value = anova_result$`F value`[1],
    df = anova_result$Df[1],
    median_C = median(lime_avg[[paste0(nutrient, "_C")]], na.rm = TRUE),
    median_SP = median(lime_avg[[paste0(nutrient, "_SP")]], na.rm = TRUE)
  )
})

# Check normality of residuals for each nutrient

lmer_results %>%
  group_by(nutrient) %>%
  do({
    model <- lmer(as.formula(paste((.$nutrient), "~ sampling_type + (1 | location)")), data = lime_log)
    shapiro_test <- shapiro.test(residuals(model))
    tibble(nutrient = .$nutrient, p_value = shapiro_test$p.value)
  })

lmer_results
```


### Boxplots:

```{r boxplots all nutrients}

lime %>%
  filter(sampling_type %in% c("C", "SP")) %>%
  pivot_longer(cols = c(ca, k, mg, mn, p, zn, p_h_2, lbc_1, lb_ceq),
               names_to = "nutrient", values_to = "value") %>%
  ggplot(aes(x = sampling_type, y = value, fill = sampling_type)) +
  geom_boxplot(outlier.shape = NA, width = 0.6) +
  geom_jitter(width = 0.2, alpha = 0.2, size = 0.8) +
  facet_wrap(~ nutrient, scales = "free_y") +
  scale_fill_manual(values = c("C" = "#1f77b4", "SP" = "#ff7f0e")) +
  labs(x = "Sampling Type", y = "Value", title = "Comparison of Nutrient Levels by Sampling Type") +
  theme_minimal(base_size = 14) +
  theme(legend.position = "none")

```




#Perry Data:

```{r Load perry data, echo=FALSE, message=FALSE, warning=FALSE}}
# Load Perry data

perry <- read_csv("../data_do_not_push/perry_soil_points_surface.csv") %>%
  clean_names() %>%
  select(label, type, x, y, lbc_1, lb_ceq, p_h_2, ca, k, mg, mn, p, zn, geometry)
```

```{r Perry assumptions}

# Define nutrient vectors
nutrients_log <- c("ca", "k", "mg", "mn", "p", "zn", "lbc_1", "lb_ceq")
nutrients <- c(nutrients_log, "p_h_2")

# Log-transform all nutrients except pH
perry_log <- perry %>%
  mutate(across(all_of(nutrients_log), log1p))

# Normality: Shapiro-Wilk test for each nutrient and group
normality_results <- map_dfr(nutrients, function(nutrient) {
  dat <- perry_log
  c_val <- dat[[nutrient]][dat$type == "CID"]
  sp_val <- dat[[nutrient]][dat$type == "SPID"]
  tibble(
    nutrient = nutrient,
    group = c("CID", "SPID"),
    p_value = c(shapiro.test(c_val)$p.value, shapiro.test(sp_val)$p.value)
  )
})

print("Shapiro-Wilk Normality Test Results:")
print(normality_results)

# Homogeneity of variance: Levene's test for each nutrient
variance_results <- map_dfr(nutrients, function(nutrient) {
  dat <- perry_log
  res <- car::leveneTest(as.formula(paste0(nutrient, " ~ type")), data = dat)
  tibble(
    nutrient = nutrient,
    p_value = res$`Pr(>F)`[1]
  )
})

print("Levene's Test for Homogeneity of Variance:")
print(variance_results)
```


# Perry To depth vs horizon based sampling

```{r}

perry_fp <- read.csv("../data_do_not_push/Perry_full_profile_points.csv")
#Point_ID has point label in it. 
perry_Ap_hz_results <- read.csv("../data_do_not_push/Perry_Ap_hz_results.csv")

#add depths to dataset:
depths <- read.csv("../data_do_not_push/Perry_FP_Ap_with_depths_cm.csv") %>%
  clean_names()

#crop data to columns 1-11 and rows 8 to 105:
perry_Ap_hz_results <- perry_Ap_hz_results[8:105, 1:11] %>%
  #make row 8 the header:
  janitor::row_to_names(row_number = 1) %>%
  clean_names()
# Extract Point_ID from the sample column
perry_Ap_hz_results_parsed <- perry_Ap_hz_results %>%
  # split "230045 - P1 - Ap" into parts
  separate(sample,
           into = c("sample_id_lab", "Point_ID", "hz"),
           sep = "\\s*-\\s*",
           remove = FALSE,
           extra = "merge",  # if there are extra dashes later, keep them in hz
           fill = "right") %>%
  mutate(
    sample_id_lab = str_trim(sample_id_lab),
    Point_ID      = str_trim(Point_ID),
    hz            = str_trim(hz)
  )

perry_fp_merged <- perry_Ap_hz_results_parsed %>%
  left_join(perry_fp, by = "Point_ID") %>%
  relocate(Point_ID, sample, sample_id_lab, hz, .before = 1) %>%
  mutate(across(c(lbc_1, lb_ceq, p_h_2, ca, k, mg, mn, p, zn), readr::parse_number))
    #merge in depths by perry_fp_merged$sample_id_lab and depths$sample_id_lab
          depths_clean <- depths %>%
            clean_names() %>%
            transmute(
              Point_ID = str_trim(point),                 # rename to match perry_fp_merged
              sample_id_lab = as.character(sample_id_lab) %>% str_trim(),
              hz = str_trim(horizon),                     # rename to match perry_fp_merged
              observational_method,
              potential_series_in_progress,
              to_depth_cm = str_trim(to_depth_cm)
            ) %>%
            # parse "0-30" into numeric columns
            separate(
              to_depth_cm,
              into = c("top_cm", "bottom_cm"),
              sep = "\\s*-\\s*",
              remove = FALSE,
              fill = "right"
            ) %>%
            mutate(
              top_cm = readr::parse_number(top_cm),
              bottom_cm = readr::parse_number(bottom_cm)
            )
          
          # 2) Make sure perry_fp_merged key columns are comparable
          perry_fp_merged2 <- perry_fp_merged %>%
            mutate(
              sample_id_lab = as.character(sample_id_lab) %>% str_trim(),
              Point_ID = str_trim(Point_ID),
              hz = str_trim(hz)
            )
          
          # 3) (Optional) sanity check: how many match on sample_id_lab?
          perry_fp_merged2 %>%
            distinct(sample_id_lab) %>%
            left_join(depths_clean %>% distinct(sample_id_lab), by = "sample_id_lab") %>%
            summarise(n_fp = n(), n_matched = sum(!is.na(sample_id_lab)))
          
          # 4) Join depths in (best key = sample_id_lab; backup keys are Point_ID + hz)
          perry_fp_merged_wdepth <- perry_fp_merged2 %>%
            left_join(
              depths_clean %>%
                select(sample_id_lab, Point_ID, hz, observational_method,
                       potential_series_in_progress, to_depth_cm, top_cm, bottom_cm),
              by = "sample_id_lab"
            )
          
          # 5) Put depth columns in a nice spot
          perry_fp_merged_wdepth <- perry_fp_merged_wdepth %>%
            relocate(to_depth_cm, top_cm, bottom_cm, .after = hz.x)
perry_fp_merged <-perry_fp_merged_wdepth    

#now datasets are merged. Now to make Ap comparison against Perry surface.
soil_vars <- c("lbc_1","lb_ceq","p_h_2","ca","k","mg","mn","p","zn")

# ---- Filter FP: Ap/Ap1/^Ap but NOT Ap2 ----
perry_fp_ap <- perry_fp_merged %>%
  mutate(hz = str_trim(hz.x)) %>%
  # filter(str_detect(hz, "^Ap"), !str_detect(hz, "^Ap2\\b")) %>%
  #filter where bottom_cm is less than or equal to 18 and where top_cm is 0:
  filter(bottom_cm <= 18, top_cm == 0) %>%
  mutate(dataset = "Full profile (Ap/Ap1)") %>%
  select(dataset, Point_ID.x, hz.x, POINT_X, POINT_Y, all_of(soil_vars))
# ---- Surface ----
perry_surface <- perry %>%
  mutate(dataset = "Surface") %>%
  select(dataset, label, type, all_of(soil_vars), x, y, geometry) %>%
  #remove type = CID
  filter(type != "CID") %>%
  #remove label points 1,23,24,34,35,36,46,60,73:
  filter(!label %in% c(1,23,24,34,35,36,46,60,73))

# ---- Long format ----
plot_df <- bind_rows(
  perry_surface %>% pivot_longer(all_of(soil_vars), names_to = "nutrient", values_to = "value"),
  perry_fp_ap     %>% pivot_longer(all_of(soil_vars), names_to = "nutrient", values_to = "value")
) %>%
  filter(!is.na(value)) %>%
  mutate(
    nutrient = factor(nutrient, levels = soil_vars),
    dataset  = factor(dataset, levels = c("Surface", "Full profile (Ap/Ap1)")),
    # log-transform everything except pH; set non-positive values to NA for log vars
    value_plot = case_when(
      nutrient == "p_h_2" ~ as.numeric(value),
      as.numeric(value) > 0 ~ log10(as.numeric(value)),
      TRUE ~ NA_real_
    ),
    y_lab = if_else(nutrient == "p_h_2", "value", "log10(value)")
  ) %>%
  filter(!is.na(value_plot))

# ---- Rainbow palette per nutrient ----
nutrient_cols <- setNames(hue_pal()(length(soil_vars)), soil_vars)


#Find p-value of differences between ap and surface:
wilcox_tbl <- plot_df %>%
  group_by(nutrient) %>%
  summarise(
    n_surface = sum(dataset == "Surface"),
    n_fp      = sum(dataset == "Full profile (Ap/Ap1)"),
    test      = list(wilcox.test(value_plot ~ dataset, data = cur_data(), exact = FALSE)),
    .groups   = "drop"
  ) %>%
  mutate(tidy = purrr::map(test, broom::tidy)) %>%
  tidyr::unnest(tidy) %>%
  select(nutrient, n_surface, n_fp, statistic, p.value)

wilcox_tbl
# 1) Compute p-values per nutrient (using the same transformed values you plotted)
pvals_df <- plot_df %>%
  group_by(nutrient) %>%
  summarise(
    p.value = wilcox.test(value_plot ~ dataset, data = cur_data(), exact = FALSE)$p.value,
    .groups = "drop"
  ) %>%
  mutate(
    p_label = paste0("Wilcoxon p = ", signif(p.value, 3))
  )

# 2) Make a per-facet y-position (near the top), so labels don't overlap the boxes
ypos_df <- plot_df %>%
  group_by(nutrient) %>%
  summarise(
    y = max(value_plot, na.rm = TRUE) * 0.95,  # near top
    .groups = "drop"
  )

ann_df <- left_join(pvals_df, ypos_df, by = "nutrient") %>%
  mutate(
    x = 1.5  # midpoint between the two boxplots
  )

# 3) Re-plot with annotations
ggplot(plot_df, aes(x = dataset, y = value_plot, fill = nutrient)) +
  geom_boxplot(width = 0.7, outlier.alpha = 0.35) +
  facet_wrap(~ nutrient, scales = "free_y", ncol = 3) +
  scale_fill_manual(values = nutrient_cols, guide = "none") +
  geom_text(
  data = ann_df,
  aes(x = -Inf, y = Inf, label = p_label),
  inherit.aes = FALSE,
  hjust = -0.1, vjust = 1.2,
  size = 3.5, fontface = "bold"
) +
  labs(
    x = NULL,
    y = NULL,
    title = "Surficial (0–15 cm) Samples vs Ap Horizon Depth Sampling — log10-transformed (except pH)"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    strip.text = element_text(face = "bold"),
    axis.text.x = element_text(angle = 25, hjust = 1)
  )

```

```{r}
#spatial comparison between ap and surface points:
# Surface SPID only (as you said)
surf <- perry_surface

# Surface coords are lon/lat
surf_sf <- st_as_sf(surf, coords = c("x","y"), crs = 4326) %>%
  st_transform(26917)   # UTM 17N meters (good for Perry)

# FP POINT_X/POINT_Y are Web Mercator meters
fp_sf <- st_as_sf(perry_fp_ap, coords = c("POINT_X","POINT_Y"), crs = 3857) %>%
  st_transform(st_crs(surf_sf))

# Nearest neighbor matching
nn_idx <- st_nearest_feature(fp_sf, surf_sf)

pairs <- fp_sf %>%
  mutate(nn = nn_idx) %>%
  bind_cols(
    st_drop_geometry(surf_sf[nn_idx, ]) %>%
      rename_with(~ paste0(.x, "_surf"))
  ) %>%
  mutate(
    dist_m = as.numeric(st_distance(geometry, st_geometry(surf_sf)[nn_idx], by_element = TRUE))
  )

# sanity check distances
summary(pairs$dist_m)


thresh <- 20  # try 25, 50, 100 as sensitivity
pairs_use <- pairs %>% filter(dist_m <= thresh)

tests <- map_dfr(soil_vars, \(v){
  df <- pairs_use %>%
    transmute(
      nutrient = v,
      fp = .data[[v]],
      surf = .data[[paste0(v, "_surf")]]
    ) %>% filter(!is.na(fp), !is.na(surf))

  tibble(
    nutrient = v,
    n_pairs = nrow(df),
    p_value = if(nrow(df) >= 5) wilcox.test(df$fp, df$surf, paired = TRUE, exact = FALSE)$p.value else NA_real_,
    median_diff = median(df$fp - df$surf, na.rm = TRUE)
  )
})

tests

#distance weighting comparison:

soil_vars <- c("lbc_1","lb_ceq","p_h_2","ca","k","mg","mn","p","zn")

# -------------------------
# 1) Clean + build sf layers
# -------------------------

# Surface: SPID only, keep coords + nutrients
surf_clean <- perry %>%
  filter(type == "SPID") %>%
  select(label, x, y, all_of(soil_vars)) %>%
  mutate(across(all_of(soil_vars), as.numeric)) %>%
  filter(!is.na(x), !is.na(y))

# lon/lat -> UTM meters for IDW
surf_sf <- st_as_sf(surf_clean, coords = c("x","y"), crs = 4326) %>%
  st_transform(26917)  # UTM 17N (meters) for Perry area

# FP Ap/Ap1 points with POINT_X/Y (Web Mercator)
fp_clean <- perry_fp_merged %>%
  mutate(hz = str_trim(hz)) %>%
  filter(str_detect(hz, "^Ap"), !str_detect(hz, "^Ap2\\b")) %>%
  select(Point_ID, hz, POINT_X, POINT_Y, all_of(soil_vars)) %>%
  mutate(across(all_of(soil_vars), as.numeric)) %>%
  filter(!is.na(POINT_X), !is.na(POINT_Y))

fp_sf <- st_as_sf(fp_clean, coords = c("POINT_X","POINT_Y"), crs = 3857) %>%
  st_transform(st_crs(surf_sf))

# -------------------------
# 2) IDW function (no kriging needed)
# -------------------------
idw_predict_at_fp <- function(values_vec, power = 2, max_dist = 200, min_n = 3) {
  # distances FP x Surface (meters)
  D <- st_distance(fp_sf, surf_sf) %>% as.matrix()
  D <- units::drop_units(D)

  preds <- rep(NA_real_, nrow(fp_sf))

  for (i in seq_len(nrow(fp_sf))) {
    d <- D[i, ]
    idx <- which(d > 0 & d <= max_dist & !is.na(values_vec))
    if (length(idx) < min_n) next
    w <- 1 / (d[idx]^power)
    preds[i] <- sum(w * values_vec[idx]) / sum(w)
  }
  preds
}

# -------------------------
# 3) Run IDW per nutrient + test FP vs IDW(surface)
# -------------------------
surf_vals <- st_drop_geometry(surf_sf)

power <- 1
max_dist <- 100  # try 100, 200, 300 as sensitivity
min_n <- 3

idw_results <- map_dfr(soil_vars, \(v) {
  pred <- idw_predict_at_fp(surf_vals[[v]], power = power, max_dist = max_dist, min_n = min_n)
  obs  <- st_drop_geometry(fp_sf)[[v]]

  df <- tibble(
    nutrient = v,
    fp_obs = obs,
    surf_idw = pred
  ) %>% filter(!is.na(fp_obs), !is.na(surf_idw)) %>%
    mutate(diff = fp_obs - surf_idw)

  tibble(
    nutrient = v,
    n_pairs = nrow(df),
    median_diff = median(df$diff, na.rm = TRUE),
    rmse = sqrt(mean((df$diff)^2, na.rm = TRUE)),
    p_value = if (nrow(df) >= 5) wilcox.test(df$diff, mu = 0, exact = FALSE)$p.value else NA_real_
  )
})

idw_results
#K and Mg did not show significant median differences, indicating stronger agreement between surface and Ap measurements for these nutrients.
library(epiR)
check_agreement <- function(nutrient_name) {
  pred <- idw_pred(surf_vals[[nutrient_name]], R = 100, power = 1, min_n = 2)
  obs  <- fp_vals[[nutrient_name]]

  df <- tibble(obs = obs, pred = pred) %>%
    filter(!is.na(obs), !is.na(pred))

  ccc <- epi.ccc(df$pred, df$obs)$rho.c[1]  # concordance correlation

  tibble(
    nutrient = nutrient_name,
    n = nrow(df),
    cor = cor(df$pred, df$obs),
    ccc = ccc,
    rmse = sqrt(mean((df$obs - df$pred)^2)),
    median_diff = median(df$obs - df$pred)
  )
}

c <- bind_rows(check_agreement("k"), check_agreement("mg"))
c
#no actual agreement. crap.can't use K or Mg. 
```

### Sensitivity analysis:

```{r}
library(sf)
library(dplyr)
library(purrr)
library(tidyr)
library(ggplot2)
library(units)

soil_vars <- c("lbc_1","lb_ceq","p_h_2","ca","k","mg","mn","p","zn")

# Choose sensitivity settings
radii  <- c(50, 75, 100, 150, 200)   # meters
powers <- c(1, 2)
min_n  <- 2

# distances FP x Surface (meters)
D <- st_distance(fp_sf, surf_sf) %>% as.matrix()
D <- units::drop_units(D)

surf_vals <- st_drop_geometry(surf_sf)
fp_vals   <- st_drop_geometry(fp_sf)

idw_pred <- function(values_vec, R, power, min_n = 2, eps = 1e-6) {
  preds <- rep(NA_real_, nrow(fp_sf))
  for (i in seq_len(nrow(fp_sf))) {
    d <- D[i, ]
    idx <- which(d <= R & !is.na(values_vec) & d > 0)
    if (length(idx) < min_n) next
    w <- 1 / ((d[idx] + eps)^power)
    preds[i] <- sum(w * values_vec[idx]) / sum(w)
  }
  preds
}

# Run sensitivity grid
sens_tbl <- expand_grid(max_dist = radii, power = powers, nutrient = soil_vars) %>%
  mutate(
    surf_idw = pmap(list(nutrient, max_dist, power), \(nutrient, max_dist, power) {
      idw_pred(surf_vals[[nutrient]], R = max_dist, power = power, min_n = min_n)
    }),
    fp_obs = map(nutrient, \(nutrient) fp_vals[[nutrient]])
  ) %>%
  mutate(
    df = map2(fp_obs, surf_idw, \(obs, pred) {
      tibble(fp_obs = obs, surf_idw = pred) %>%
        filter(!is.na(fp_obs), !is.na(surf_idw)) %>%
        mutate(diff = fp_obs - surf_idw)
    })
  ) %>%
  mutate(
    n_pairs = map_int(df, nrow),
    median_diff = map_dbl(df, ~ median(.x$diff, na.rm = TRUE)),
    rmse = map_dbl(df, ~ sqrt(mean((.x$diff)^2, na.rm = TRUE))),
    p_value = map_dbl(df, ~ if (nrow(.x) >= 5) wilcox.test(.x$diff, mu = 0, exact = FALSE)$p.value else NA_real_)
  ) %>%
  select(max_dist, power, nutrient, n_pairs, median_diff, rmse, p_value)

sens_tbl

ggplot(sens_tbl, aes(x = max_dist, y = median_diff, group = factor(power), linetype = factor(power))) +
  geom_hline(yintercept = 0, alpha = 0.4) +
  geom_line() +
  geom_point(aes(size = n_pairs)) +
  facet_wrap(~ nutrient, scales = "free_y", ncol = 3) +
  scale_x_continuous(breaks = sort(unique(sens_tbl$max_dist))) +
  labs(
    x = "IDW search radius (m)",
    y = "Median(FP Ap − IDW(surface))",
    linetype = "IDW power",
    size = "Matched FP points (n)",
    title = "IDW sensitivity: FP Ap vs distance-weighted surface estimates",
    subtitle = "Points sized by n_pairs; lines compare power=1 vs power=2"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    strip.text = element_text(face = "bold")
  )

ggplot(sens_tbl, aes(x = max_dist, y = rmse, group = factor(power), linetype = factor(power))) +
  geom_line() +
  geom_point(aes(size = n_pairs)) +
  facet_wrap(~ nutrient, scales = "free_y", ncol = 3) +
  scale_x_continuous(breaks = sort(unique(sens_tbl$max_dist))) +
  labs(
    x = "IDW search radius (m)",
    y = "RMSE(FP Ap − IDW(surface))",
    linetype = "IDW power",
    size = "Matched FP points (n)",
    title = "IDW sensitivity: mismatch magnitude vs radius"
  ) +
  theme_minimal(base_size = 12) +
  theme(strip.text = element_text(face = "bold"))
#dist = 100m best, power 1 best
```




```{r}
#wilcoxon:
#log:
  # perry_log <- perry %>%
  #   mutate(across(c(ca, k, mg, mn, p, zn, p_h_2, lbc_1, lb_ceq), log1p))
perry_wide <- perry %>%
  select(label, type, all_of(nutrients)) %>%
  pivot_wider(names_from = type, values_from = all_of(nutrients))  # gives e.g. ca_CID and ca_SPID

wilcox_results <- map_dfr(nutrients, function(nutrient) {
  # Create the two paired vectors
  x <- perry_wide[[paste0(nutrient, "_CID")]]
  y <- perry_wide[[paste0(nutrient, "_SPID")]]

  # Remove any NA pairs
  complete_idx <- complete.cases(x, y)
  x <- x[complete_idx]
  y <- y[complete_idx]

  # Wilcoxon signed-rank test (paired)
  wt <- wilcox.test(x, y, paired = TRUE, exact = FALSE)

  # Output a row
  tibble(
    nutrient = nutrient,
    p_value = wt$p.value,
    median_CID = median(x, na.rm = TRUE),
    median_SPID = median(y, na.rm = TRUE),
    W = wt$statistic
  )
})
wilcox_results
```

```{r LMER}
#do same but with lmer 
#log:
  # perry_log <- perry %>%
  #   mutate(across(c(ca, k, mg, mn, p, zn, p_h_2, lbc_1, lb_ceq), log1p))
lmer_perry_results <- map_dfr(nutrients, function(nutrient) {
  formula <- as.formula(paste(nutrient, "~ type + (1 | label)"))
  model <- lmer(formula, data = perry)
  
  anova_result <- anova(model)
  
  tibble(
    nutrient = nutrient,
    p_value = anova_result$`Pr(>F)`[1],
    F_value = anova_result$`F value`[1],
    df = anova_result$Df[1],
    median_CID = median(perry_wide[[paste0(nutrient, "_CID")]], na.rm = TRUE),
    median_SPID = median(perry_wide[[paste0(nutrient, "_SPID")]], na.rm = TRUE)
  )
})
lmer_perry_results
```

### Formatting:
```{r}
# Helper function for significance stars
sig_stars <- function(p) {
  dplyr::case_when(
    p < 0.001 ~ "***",
    p < 0.01  ~ "**",
    p < 0.05  ~ "*",
    TRUE      ~ ""
  )
}

# Liming Wilcoxon table
liming_wilcox_table <- results %>%
  mutate(significance = sig_stars(p_value)) %>%
  select(nutrient, median_C, median_SP, W, p_value, significance) %>%
  arrange(factor(nutrient, levels = nutrients))

# Liming LMER table
liming_lmer_table <- lmer_results %>%
  mutate(significance = sig_stars(p_value)) %>%
  select(nutrient, F_value, p_value, significance) %>%
  arrange(factor(nutrient, levels = nutrients))

# Perry Wilcoxon table
perry_wilcox_table <- wilcox_results %>%
  mutate(significance = sig_stars(p_value)) %>%
  select(nutrient, median_CID, median_SPID, W, p_value, significance) %>%
  rename(median_C = median_CID, median_SP = median_SPID) %>%
  arrange(factor(nutrient, levels = nutrients))

# Perry LMER table
perry_lmer_table <- lmer_perry_results %>%
  mutate(significance = sig_stars(p_value)) %>%
  select(nutrient, median_CID, median_SPID, F_value, p_value, significance) %>%
  rename(median_C = median_CID, median_SP = median_SPID) %>%
  arrange(factor(nutrient, levels = nutrients))

# Print all tables using knitr::kable for neat output
library(knitr)
kable(liming_wilcox_table, caption = "Liming Study: Wilcoxon Signed-Rank Test")
kable(liming_lmer_table, caption = "Liming Study: Linear Mixed-Effects Model (log)")
kable(perry_wilcox_table, caption = "Perry Study: Wilcoxon Signed-Rank Test")
kable(perry_lmer_table, caption = "Perry Study: Linear Mixed-Effects Model")
```

### Log transform and test:

```{r}
# ---- Define nutrient names ----
nutrients_log <- c("ca", "k", "mg", "mn", "p", "zn", "lbc_1", "lb_ceq")
nutrients <- c(nutrients_log, "p_h_2")  # pH remains untransformed

# ---- Log-transform nutrients except pH ----
lime_log <- lime %>%
  mutate(across(all_of(nutrients_log), log1p))

perry_log <- perry %>%
  mutate(across(all_of(nutrients_log), log1p))

# ---- Significance stars helper ----
sig_stars <- function(p) {
  dplyr::case_when(
    p < 0.001 ~ "***",
    p < 0.01  ~ "**",
    p < 0.05  ~ "*",
    TRUE      ~ ""
  )
}

# ---- Shapiro-Wilk test function ----
shapiro_cluster_check <- function(df, nutrient_name, group_var) {
  df %>%
    filter(!is.na(.data[[nutrient_name]])) %>%
    group_by(.data[[group_var]]) %>%
    summarise(
      p_value = shapiro.test(.data[[nutrient_name]])$p.value,
      .groups = "drop"
    ) %>%
    mutate(
      nutrient = nutrient_name,
      group = .data[[group_var]],
      sig = sig_stars(p_value)
    )
}

# ---- Run normality tests ----
log_normality_lime <- map_dfr(nutrients, ~ shapiro_cluster_check(lime_log, .x, "sampling_type"))
log_normality_perry <- map_dfr(nutrients, ~ shapiro_cluster_check(perry_log, .x, "type"))

# ---- Display results ----
library(knitr)
kable(log_normality_lime, caption = "Shapiro-Wilk Normality Test (Log-Transformed Nutrients + Raw pH) — Lime")
kable(log_normality_perry, caption = "Shapiro-Wilk Normality Test (Log-Transformed Nutrients + Raw pH) — Perry")


# ---- Liming: Wilcoxon (paired, log except pH) ----
lime_avg_log <- lime_log %>%
  mutate(sampling_type = ifelse(type %in% c("A", "B"), "SP", sampling_type)) %>%
  group_by(location, sampling_type) %>%
  summarise(across(all_of(nutrients), mean, na.rm = TRUE), .groups = "drop")

lime_wide_log <- lime_avg_log %>%
  pivot_wider(names_from = sampling_type, values_from = all_of(nutrients))

liming_wilcox_results <- map_dfr(nutrients, function(nutrient) {
  C_col <- lime_wide_log[[paste0(nutrient, "_C")]]
  SP_col <- lime_wide_log[[paste0(nutrient, "_SP")]]
  test <- wilcox.test(C_col, SP_col, paired = TRUE, exact = FALSE)
  tibble(
    nutrient = nutrient,
    p_value = test$p.value,
    median_C = median(C_col, na.rm = TRUE),
    median_SP = median(SP_col, na.rm = TRUE),
    W = test$statistic
  )
})

# ---- Liming: LMER (log except pH) ----
liming_lmer_results <- map_dfr(nutrients, function(nutrient) {
  formula <- as.formula(paste(nutrient, "~ sampling_type + (1 | location)"))
  model <- lmer(formula, data = lime_log)
  anova_result <- anova(model)
  tibble(
    nutrient = nutrient,
    p_value = anova_result$`Pr(>F)`[1],
    F_value = anova_result$`F value`[1]
  )
})

# ---- Perry: Wilcoxon (paired, log except pH) ----
perry_wide_log <- perry_log %>%
  select(label, type, all_of(nutrients)) %>%
  pivot_wider(names_from = type, values_from = all_of(nutrients))

perry_wilcox_results <- map_dfr(nutrients, function(nutrient) {
  x <- perry_wide_log[[paste0(nutrient, "_CID")]]
  y <- perry_wide_log[[paste0(nutrient, "_SPID")]]
  complete_idx <- complete.cases(x, y)
  x <- x[complete_idx]
  y <- y[complete_idx]
  wt <- wilcox.test(x, y, paired = TRUE, exact = FALSE)
  tibble(
    nutrient = nutrient,
    p_value = wt$p.value,
    median_CID = median(x, na.rm = TRUE),
    median_SPID = median(y, na.rm = TRUE),
    W = wt$statistic
  )
})

# ---- Perry: LMER (log except pH) ----
perry_lmer_results <- map_dfr(nutrients, function(nutrient) {
  formula <- as.formula(paste(nutrient, "~ type + (1 | label)"))
  model <- lmer(formula, data = perry_log)
  anova_result <- anova(model)
  tibble(
    nutrient = nutrient,
    p_value = anova_result$`Pr(>F)`[1],
    F_value = anova_result$`F value`[1]
  )
})

# ---- Format and print all tables ----
liming_wilcox_table <- liming_wilcox_results %>%
  mutate(significance = sig_stars(p_value)) %>%
  select(nutrient, median_C, median_SP, W, p_value, significance) %>%
  arrange(factor(nutrient, levels = nutrients))

liming_lmer_table <- liming_lmer_results %>%
  mutate(significance = sig_stars(p_value)) %>%
  select(nutrient, F_value, p_value, significance) %>%
  arrange(factor(nutrient, levels = nutrients))

perry_wilcox_table <- perry_wilcox_results %>%
  mutate(significance = sig_stars(p_value)) %>%
  select(nutrient, median_CID, median_SPID, W, p_value, significance) %>%
  rename(median_C = median_CID, median_SP = median_SPID) %>%
  arrange(factor(nutrient, levels = nutrients))

perry_lmer_table <- perry_lmer_results %>%
  mutate(significance = sig_stars(p_value)) %>%
  select(nutrient, F_value, p_value, significance) %>%
  arrange(factor(nutrient, levels = nutrients))

kable(liming_wilcox_table, caption = "Liming Study: Wilcoxon Signed-Rank Test (log-transformed except pH)")
kable(liming_lmer_table, caption = "Liming Study: Linear Mixed-Effects Model (log-transformed except pH)")
kable(perry_wilcox_table, caption = "Perry Study: Wilcoxon Signed-Rank Test (log-transformed except pH)")
kable(perry_lmer_table, caption = "Perry Study: Linear Mixed-Effects Model (log-transformed except pH)")
```


